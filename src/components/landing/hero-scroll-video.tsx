...
const HERO_SCROLL_DISTANCE_PX = 4800;
const PRIORITY_FRAMES = 24;
const PRELOAD_CONCURRENCY = 6;
const PRIORITY_PRELOAD_CONCURRENCY = 5;
const ACTIVE_FRAME_WARM_RANGE = 5;
const FRAME_LERP_FACTOR = 0.18;
const MOTION_BLUR_TRIGGER_DELTA = 0.6;
const MOTION_BLUR_ALPHA = 0.08;
const FRAME_REDRAW_EPSILON = 0.02;
...
export function HeroScrollVideo() {
  ...
  useEffect(() => {
    ...
    const drawFrame = (targetFrame: number, force = false) => {
      ...
      if (!force && Math.abs(safeFrame - lastRenderedFrameValue) < FRAME_REDRAW_EPSILON) {
        return;
      }
    ...
    const resizeCanvas = () => {
      ...
      const sizeChanged = width !== cachedWidth || height !== cachedHeight || dpr !== cachedDpr;
      if (!sizeChanged) {
        return;
      }
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-10 bg-linear-to-t from-black/62 via-black/28 to-black/12" />
      <div className="absolute inset-0 z-10 bg-[radial-gradient(circle_at_20%_16%,rgba(249,115,22,0.18),transparent_36%)]" />
        ...
        <div className="hero-content max-w-2xl space-y-6">
          ...
          <div className="flex flex-wrap gap-3">
            <Button
              asChild
              size="lg"
              className="rounded-xl border border-primary/70 bg-primary px-7 font-semibold text-primary-foreground shadow-[0_14px_45px_rgba(249,115,22,0.35)]"
            >
